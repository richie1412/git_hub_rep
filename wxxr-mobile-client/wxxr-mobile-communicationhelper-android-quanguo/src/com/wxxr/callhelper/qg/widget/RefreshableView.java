...
public class RefreshableView extends LinearLayout implements OnTouchListener {
	...
	private ArrayList<AsyncTask<Void, Integer, Integer>> pendingTasks = new ArrayList<AsyncTask<Void, Integer, Integer>>();
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (changed && (headerLayoutParams == null)) {
			...
			if (log.isDebugEnabled()) {
				log.debug("init Layout params, hideHeaderHeight :"
						+ hideHeaderHeight);
			}
			...
			if (log.isDebugEnabled()) {
				log.debug("onLayout, hideHeaderHeight :"
	...
	private void setIsAbleToPull(View view, MotionEvent event) {
		...
		if (pullableView.getView() instanceof ListView) {
			...
			if (firstChild != null) {
				int firstVisiblePos = ((ListView) pullableView.getView())
	...
	private void updateHeaderView() {
		if (log.isDebugEnabled()) {
			log.debug("updateHeaderView, current status :" + currentStatus
