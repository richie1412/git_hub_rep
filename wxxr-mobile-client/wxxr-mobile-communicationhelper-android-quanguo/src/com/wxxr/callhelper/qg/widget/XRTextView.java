...
public class XRTextView extends TextView{ 
 ...
 private float LineSpacing = 1.0f;//行与行的间距 
 ...
 public XRTextView(Context context, AttributeSet attrs) { 
  ...
  textColor = this.getPaint().getColor();
  ...
  paint1.setColor(Color.rgb(0xbd, 0xbd, 0xc5));
...
//  super.onDraw(canvas); 
  ...
  textShowWidth=view.getMeasuredWidth()-paddingLeft - paddingRight - marginLeft - marginRight-paint1.measureText(""); 
  int lineCount = 0; 
   
  text = ToDBC(this.getText().toString());//.replaceAll("\n", "\r\n"); 
  if(text==null)return; 
 ...
 public void setMYTextColro(float textSize) { 
	  this.textSize = textSize; 
	  paint1.setColor(Color.rgb(0xbd, 0xbd, 0xc5));
