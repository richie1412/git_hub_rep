...
import com.wxxr.callhelper.qg.db.HuiZhiOpenHelper;
...
public class HuiZhiBaoGaoDao {
	...
	public void deleteSMSAccordingtoNumberAndMonth(String telnumber,
	 ...
	 /**
	  * 
	  * @param telnumber  有值，表示是正常的，数据，内容里有手机号码，按照号码来删除
	  * @param port      telnumber，没值，只能按照 port 来删除了
	  */
		public void deleteAll(String telnumber,
				String port) {
			...
			if (db.isOpen()) {
				...
				db.execSQL(
						"delete from tablehuizhibaogao where tosomebody = ? ",
						new Object[] {telnumber});
				}else{
				db.execSQL(
							"delete from tablehuizhibaogao where smsnumber = ? ",
							new Object[] {port});
				}
				db.close();
			}
