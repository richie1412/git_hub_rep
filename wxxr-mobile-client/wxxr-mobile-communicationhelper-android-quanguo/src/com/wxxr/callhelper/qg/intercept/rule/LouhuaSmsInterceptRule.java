 ...
 *
 */
public class LouhuaSmsInterceptRule implements ILouhuaSmsInterceptRule {
	private static final Trace log = Trace.register(LouhuaSmsInterceptRule.class);

	private String name;
	private List<Pattern> ps=new ArrayList<Pattern>();
	private Pattern px = Pattern.compile("\\s*|\t|\r|\n");

	public  LouhuaSmsInterceptRule(String name){
		this.name=name;
	}
	/**
	 * @param name 省份代码
	 * @param rules 正则表达式列表
	 */
	public  LouhuaSmsInterceptRule(String name, String[] rules){
		this.name=name;
